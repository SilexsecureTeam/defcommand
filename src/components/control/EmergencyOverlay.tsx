...
export default function EmergencyOverlay() {
  ...
  const enrichedEmergencies = useMemo(() => {
    return emergencies
      .map((e) => ({ ...e, sender: contactsById[e.senderId] ?? null }))
      .filter((e) => e.sender); // only known contacts
  }, [emergencies, contactsById]);

  const selectedEmergencyWithSender = useMemo(() => {
    if (!selectedEmergency) return null;
    const sender = contactsById[selectedEmergency.senderId] ?? null;
    return sender ? { ...selectedEmergency, sender } : null;
  }, [selectedEmergency, contactsById]);

  if (!isPanelOpen || !selectedEmergencyWithSender) return null;
